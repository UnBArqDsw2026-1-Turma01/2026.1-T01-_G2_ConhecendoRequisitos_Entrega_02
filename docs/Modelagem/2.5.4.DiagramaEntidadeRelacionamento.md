...
A construção do DER exigiu a tomada de decisões para garantir que o modelo representasse fielmente o domínio do ConhecendoRequisitos. As principais escolhas e suas justificativas foram:

- **Herança (Generalização/Especialização):** A hierarquia Usuario → Administrador / Aluno foi representada como uma especialização total e exclusiva (disjunta). O grupo optou por essa abordagem porque, no atual contexto de negócios, um usuário assume papéis estritamente divididos, sem interseção de papéis.
- **Entidades Fracas vs. Fortes:** As entidades Modulo, Conteudo e Alternativa foram identificadas como fracas. A justificativa é que um `Modulo` não possui significado sem uma `Trilha` proprietária, assim como o `Conteudo` em relação ao módulo. Em contrapartida, a decisão de manter `Questao` como entidade forte reflete um senso de reaproveitamento: questões têm valor isolado e poderão ser utilizadas para formar um "Banco de Questões" unificado futuramente, não dependendo de apenas um único Quiz para existir.
- **Atributos Derivados:** Atributos como `qtdModulos`, `qtdConteudos` e `qtdQuestoes` foram intencionalmente omitidos. A decisão visa manter o modelo limpo de redundâncias, priorizando a normalização. Dados quantitativos devem ser obtidos dinamicamente por consultas estruturadas nos relacionamentos.
- **Relacionamentos em Alto Nível:** Os relacionamentos Aluno–Trilha ("estuda") e Aluno–Quiz ("realiza") foram mantidos como N:N simples. Em vez de povoar o DER com entidades associativas complexas de progresso e estado, decidiu-se delegar esse detalhamento ao Diagrama Lógico, separando as preocupações puramente conceituais daquelas voltadas para a persistência.
- **Omissão de Chaves Estrangeiras:** Como aponta a literatura clássica de Modelagem Conceitual (ELMASRI; NAVATHE, 2011), optou-se rigorosamente por não representar chaves estrangeiras no DER, pois a integridade é definida visualmente através dos relacionamentos em losango. Além disso, as diretrizes do mapeamento seguiram os padrões apresentados por Serrano (2025) ao longo da disciplina.
...
**Tabela 3: Notação visual do DER**
...
Na elaboração do DER, o senso crítico foi aplicado principalmente nas decisões de modelagem conceitual, como a definição de entidades fracas, a especialização de usuários e o nível de abstração dos relacionamentos N:N. A equipe precisou equilibrar fidelidade ao domínio com simplicidade do modelo, evitando antecipar detalhes de implementação que pertencem ao diagrama lógico.

O trabalho em equipe foi essencial para justificar tecnicamente cada escolha, confrontando alternativas de modelagem com base na literatura e nas regras de negócio do projeto. As discussões coletivas e revisões do diagrama contribuíram para um artefato mais consistente, didático e preparado para servir de base confiável à etapa relacional.
...
> CHEN, Peter. **The Entity-Relationship Model: Toward a Unified View of Data**. ACM Transactions on Database Systems, v. 1, n. 1, p. 9–36, 1976. Disponível em: <https://dl.acm.org/doi/10.1145/320434.320440>. Acesso em: 20 abr. 2026.
...
> DRAW.IO. **Diagrams.net — Free Online Diagram Software**. Disponível em: <https://www.drawio.com>. Acesso em: 20 abr. 2026.
...
| 1.1    | 22/04 | Ajustes na metodologia e referências | [Arthur Oliveira](https://github.com/arthurevg) |                                                  |                     |
