...
Neste modelo, a herança (generalização/especialização) de Usuario foi mapeada utilizando a estratégia de **tabelas separadas com chave estrangeira**, mantendo uma tabela para a superclasse e tabelas adicionais para cada especialização. As entidades associativas oriundas dos relacionamentos N:N foram transformadas em tabelas próprias com chaves estrangeiras compostas referenciando as tabelas participantes.

### 1.1. Metodologia

A transição do modelo conceitual para o relacional exigiu escolhas para garantir a integridade referencial, normalização e desempenho:

- **Estratégia de Herança:** Foi evitada a estratégia de Tabela Única (_Single-Table_) para impedir anomalias e proliferação de valores nulos, conforme os princípios abordados por Elmasri e Navathe (2011) e Serrano (2025). Com tabelas separadas para `Aluno` e `Administrador`, assegurou-se uma integridade forte sobre os atributos e restrições exclusivos de cada papel.
- **Chaves Artificiais (Surrogate Keys):** Nas tabelas geradas a partir de entidades fracas (`Modulo`, `Conteudo`, `Alternativa`), adotou-se intencionalmente chaves primárias artificiais auto-incrementais (`id`). A quebra da regra tradicional (que exigiria chaves compostas pesadas) foi uma decisão para melhorar o desempenho, simplificando a propagação de FKs em tabelas dependentes e tornando os relacionamentos e buscas (`JOINs`) mais eficientes.
- **Gestão de Estado nas Tabelas Associativas:** Para gerenciar os relacionamentos N:N, as tabelas de progresso receberam restrições `UNIQUE` compostas no banco de dados. A decisão foi delegar ao SGBD (e não apenas à aplicação) a garantia de que um aluno não duplique seu histórico num módulo. Já em `TentativaQuiz`, essa restrição foi omitida, modelando corretamente a realidade de que um usuário pode ter infinitas tentativas para o mesmo quiz.
- **Políticas de Deleção (ON DELETE CASCADE vs SET NULL):** O CASCADE foi adotado pois a maior parte do domínio possui ciclo de vida acoplado (ex: remover uma trilha anula o sentido de seus módulos). Contudo, aplicou-se `SET NULL` na ligação do `Quiz` com o `Modulo`, pois se reconheceu que um quiz (e suas questões de banco) possui utilidade própria mesmo se desvinculado de um módulo de trilha.
...
A elaboração do Diagrama Lógico exigiu senso crítico para converter o modelo conceitual em estruturas relacionais consistentes, com decisões claras sobre chaves primárias, estrangeiras, restrições de unicidade e políticas de deleção. O grupo avaliou impactos de cada escolha sobre integridade, desempenho e evolução do banco, buscando um modelo equilibrado entre normalização e viabilidade de implementação.

No trabalho em equipe, a colaboração ocorreu por meio de validações sucessivas entre regras de negócio, cardinalidades e restrições do esquema SQL. Essa revisão conjunta foi importante para identificar inconsistências antes da implementação física, além de garantir que o artefato final refletisse de forma alinhada os objetivos acadêmicos e técnicos do projeto.
...
| 1.1    | 22/04 | Ajustes na metodologia e referências | [Arthur Oliveira](https://github.com/arthurevg) | [Yan Matheus](https://github.com/Yanmatheus0812) | Revisado e aprovado |
